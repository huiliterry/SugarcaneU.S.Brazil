...
var trainingSampleConstr = function(startDateTrain,endDateTrain,startDateTrain_NS,endDateTrain_NS,FloridaLabels,LouisianaLabels,order,S2L89_CLD_PRB_THRESH){
  ...
  print('Florida_trainingSample',Florida_trainingSample.size());
  ...
  print('Louisiana_trainingSample',Louisiana_trainingSample.size());
...
var classification = function(startDate,endDate,targetAera,classifier,order,S2L89_CLD_PRB_THRESH){
  var S2_CLOUD_FILTER = 100;//60;//60-Florida 100-other
  // var S2L89_CLD_PRB_THRESH = 40;//Nebraska-20 Brazile-40 46 50
  var S2_NIR_DRK_THRESH = 0.25;//0.10//0.22
  var S2_CLD_PRJ_DIST = 1;
  var S2_BUFFER = 20; 
  
  var targetName = targetAera.first().get('NAME').getInfo().replace(/ /g,'');
  targetAera = targetAera.geometry();
  Map.addLayer(targetAera,{},targetName+'targetAera');
  ...
  Map.addLayer(preProsS2.select('NDVI'),{},'preProsS2',false);
  
  var stdDev = preProsS2.select('NDVI').reduce(ee.Reducer.stdDev());
  Map.addLayer(stdDev,{},'stdDev',false);
  var stdDevLand = stdDev.gt(0.15);
  
  var cropMask = ee.ImageCollection("ESA/WorldCover/v200").first().clip(targetAera).eq(40);
  var CDL = ee.ImageCollection('USDA/NASS/CDL').filter(ee.Filter.date('2022-01-01','2022-12-31')).first().clip(targetAera);
  var cropLandMask = CDL.select('cultivated').remap([1,2],[0,1]).selfMask();
  
  // generating harmonic coefficients
  var classificationOutput = ResCoesCosine(order,preProsS2)
    .toBands()
    .classify(classifier)
    .clip(targetAera);
  print('classificationOutput',classificationOutput);
  Map.addLayer(classificationOutput,{min:0,max:1,palette:['black', 'yellow']},'classificationOutput');

  Export.image.toDrive({
    image: classificationOutput,
    description:'smallTest',
    scale: 10,
    crs: 'epsg:4326',
    region: targetAera,
...
/***********/
var S2L89_CLD_PRB_THRESH = 65;
var order = 12;
var intervalValue = 15;
var band = ['B3','B4','B5','NDWI'];
/*Training-USA*/
var samples2020_9 = trainingSampleConstr('2019-09-30','2020-09-30','2019-09-30','2020-09-30',RandomSugarcane1000_03,LafourcheSugarcane1000_03,order,S2L89_CLD_PRB_THRESH);
var samples2020_8 = trainingSampleConstr('2019-08-30','2020-08-30','2019-08-30','2020-08-30',RandomSugarcane1000_03,LafourcheSugarcane1000_03,order,S2L89_CLD_PRB_THRESH);
var samples2020_7 = trainingSampleConstr('2019-07-30','2020-07-30','2019-07-30','2020-07-30',RandomSugarcane1000_03,LafourcheSugarcane1000_03,order,S2L89_CLD_PRB_THRESH);

var samples2021_9 = trainingSampleConstr('2020-09-30','2021-09-30','2020-09-30','2021-09-30',RandomSugarcane1000_02,LafourcheSugarcane1000_02,order,S2L89_CLD_PRB_THRESH);
var samples2021_8 = trainingSampleConstr('2020-08-30','2021-08-30','2020-08-30','2021-08-30',RandomSugarcane1000_02,LafourcheSugarcane1000_02,order,S2L89_CLD_PRB_THRESH);
var samples2021_7 = trainingSampleConstr('2020-07-30','2021-07-30','2020-07-30','2021-07-30',RandomSugarcane1000_02,LafourcheSugarcane1000_02,order,S2L89_CLD_PRB_THRESH);


var samples = samples2020_9.merge(samples2020_8).merge(samples2020_7)
      .merge(samples2021_9).merge(samples2021_8).merge(samples2021_7);

var classifier = classifierConstr(samples,bandsName);

band = band.toString().replace(/\,/g,'');
Export.classifier.toAsset({
  classifier: classifier,
  description: assetId,
  assetId: 'YOURCLASSIFIER'
});
