...
var classification = function(startDate,endDate,targetAera,classifier,order,S2L89_CLD_PRB_THRESH){
  ...
  var S2_BUFFER = 20; //100
  // var intervalValue =16;
  // var order = 6;//7//10;
  ...
  var GCVImaxMask = GCVImax.gte(2.0).and(GCVImax.lte(5));
...
/*Classification-Sao Paulo*/ 
var classifier = ee.Classifier.load('YOURCLASSIFIER');

var SaoPaulo = ee.FeatureCollection('FAO/GAUL/2015/level1')
                .filter(ee.Filter.eq('ADM0_NAME','Brazil'))
                .filter(ee.Filter.eq('ADM1_NAME','Sao Paulo'))
                .map(function(fea){return fea.set("NAME","SaoPaulo")});

var date = ['-04-01','-05-01','-06-01','-07-01','-08-01'];


for (var d=0; d <5; d++){
  var startDateTarget = '2021' + date[d], endDateTarget = '2022' + date[d];
  var cla01 = classification(startDateTarget,endDateTarget,SaoPauloMap001,classifier,order,S2L89_CLD_PRB_THRESH);
  var cla02 = classification(startDateTarget,endDateTarget,SaoPauloMap002,classifier,order,S2L89_CLD_PRB_THRESH);
  var cla03 = classification(startDateTarget,endDateTarget,SaoPauloMap003,classifier,order,S2L89_CLD_PRB_THRESH);
  var cla04 = classification(startDateTarget,endDateTarget,SaoPauloMap004,classifier,order,S2L89_CLD_PRB_THRESH);
  
  var finalCla = ee.ImageCollection([cla01,cla02,cla03,cla04]).mosaic().toByte();
  
  var bandList = band.toString().replace(/\,/g,'');
